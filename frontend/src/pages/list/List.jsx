...
const TopMoviesList = () => {
  ...
  const getData = async () => {
    const response = await axios.post(
      "http://localhost:3000/api/list/get/" + list,
      {
  ...
  const handleChange = (e) => {
    ...
    setList(value);
    if (value === "all") setMovies(plannedList.concat(watchedList));
    else if (value === "watched") setMovies(watchedList);
    else if (value === "planned") setMovies(plannedList);
    set_i(1);
  };
