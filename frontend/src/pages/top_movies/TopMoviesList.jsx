...
const TopMoviesList = () => {
  ...
  const getGenresDB = async () => {
    const response = await axios.get(
      "http://localhost:8000/api/movies/top_db",
      { headers: { "Content-Type": "application/json", withCredentials: true } }
    );
  ...
  const getUserList = async () => {
    ...
    if (user != null) {
      const response = await axios.post(
        "http://localhost:3000/api/list/get/all",
        {
          headers: {
      ...
      <div className="shadow-lg h-[100%] w-[100%] flex items-center flex-grow bg-[#ffe7cc]">
        <div className="overflow-y-auto flex flex-col w-[83.333333%] shadow-lg  mr-16 h-5/6 flex-grow rounded-br-3xl rounded-tr-3xl border-b-2 border-r-2 border-t-2 border-[#fea1a1] bg-[#f9fbe7] items-start">
          <div className="flex flex-col items-center w-[100%]">
            <button
              className="mr-[70%] mt-6 border shadow-md hover:shadow-lg border-[#fea1a1] bg-[#f9fbe7] text-[#fea1a1] pt-2 pb-2 pl-6 pr-6 rounded-3xl"
              onClick={handleChart}
            >
              Chart
            </button>
            <select
              className="mt-6 border shadow-md hover:shadow-lg border-[#fea1a1] bg-[#f9fbe7] text-[#fea1a1] pt-2 pb-2 pl-6 pr-6 rounded-3xl"
              value={genre}
